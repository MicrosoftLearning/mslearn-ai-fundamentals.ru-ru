...
1. В браузере перейдите к разделу `https://ai.azure.com/managementCenter/allResources` и нажмите кнопку **"Создать".** Затем выберите параметр для создания ресурса** центра искусственного **интеллекта.
...
1. Разверните *дополнительные параметры* , чтобы указать следующие параметры для проекта:
    ...
    Нажмите кнопку **создания**. Дождитесь создания проекта и центра. Это может занять несколько минут.
...
1. **На странице "Добавление подписей к изображениям**" в *меню выбора подключенных служб* ИИ Azure обратите внимание *, что созданный ресурс служб* ИИ Azure выбран. Вам не нужно вносить изменения. 

    >*Примечание.* Если вы не настроили допустимое расположение ресурсов во время создания ресурса, вам может потребоваться создать новый ресурс служб искусственного интеллекта Azure, который находится в допустимом регионе. Чтобы продолжить работу с лабораторией, необходимо создать новый ресурс.  

1. Скачайте **image-analysis.zip** , открыв URL-адрес на новой вкладке браузера. С помощью URL-адреса `https://aka.ms/mslearn-images-for-analysis` нужно автоматически скачать папку на компьютере. 

1. Перейдите в папку *downloads* на компьютере, а затем определите скачаемую папку. Щелкните правой кнопкой мыши скачаемую папку. Выберите *"Извлечь все...*". Затем выберите *"Извлечь"* , чтобы распакуировать его содержимое. На экране появится распакованная папка.  

1. В распакованной папке найдите файл с именем **store-camera-1.jpg**; который содержит следующее изображение:
...
1. **Отправьте изображение store-camera-1.jpg**, нажав кнопку *"Обзор файла*". Изображение можно найти в папке "Загрузки* файловой *системы".
