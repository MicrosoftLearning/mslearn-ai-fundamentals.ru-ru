...
1. В нижней **половине страницы "Обзор** " для ресурса "Поиск ИИ Azure" выберите вкладку **"Индексаторы** ". На этой вкладке показан только что созданный **индексатор** кофе. Немного подождите и нажимайте кнопку **&orarr;Обновить**, пока значение параметра **Состояние** не изменится на "Выполнено".
...
Этот простой индекс поиска только некоторые возможности azure AI служба . Дополнительные сведения о том, что можно сделать с этой службой, см[. на странице](/azure/search/search-what-is-azure-search) azure AI служба .
